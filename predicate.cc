...
//think about cuts based on interface
    ...
    //can we skip until the next hw for tags?
    //for example if we receive a filter with hw 8, chan we start the
    //superset check from 15? In theroy no, but which is the probability
    //have problems?
